...
export default function AmlGraph() {
  ...
  useEffect(() => {
    ...
    function update(){
      ...
      node.selectAll("text")
        .filter((d,i,n)=>i===1)
        .text(d=>`Risk ${(risk[d.id]*100).toFixed(0)}%`);

      explanationRef.current.innerText =
        mode === "rule"
    ...
    modeSelectRef.current.onchange = e => {
      mode = e.target.value;
      gnnRisk = { ...gnnRiskBase };
      update();
    };
  ...
  return (
    <div className="bg-[#0b1020] text-white min-h-screen">
      <header className="flex items-center gap-5 px-6 py-3 bg-[#0f172a]">
        <h1 className="text-lg font-semibold">AML Smurfing Detection</h1>
        <select
          ref={modeSelectRef}
          className="bg-slate-800 border border-slate-600 rounded px-3 py-1 cursor-pointer"
        >
      ...
      <div
        ref={explanationRef}
        className="px-6 py-2 text-sm bg-[#020617] opacity-90"
      />
